        ...
        for l in range(depth):
            ...
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l+1]):
                    ...
                    if tick == True:
                        ax.tick_params(axis="y", direction="out", pad=2, labelsize=5)
                        ax.tick_params(axis="x", direction="out", pad=2, labelsize=5)
                        ...
                        plt.xticks([x_min, x_max], ['%.3f' % x_min, '%.3f' % x_max])
                        plt.yticks([y_min, y_max], ['%.3f' % y_min, '%.3f' % y_max])
                    else:
                        plt.xticks([])
                        plt.yticks([])
                    if alpha_mask == 1:
                    ...
                    plt.plot(self.acts[l][:, i][rank].cpu().detach().numpy(), self.spline_postacts[l][:, j, i][rank].cpu().detach().numpy(), color=color, lw=1)
                    if sample == True:
                        plt.scatter(self.acts[l][:, i][rank].cpu().detach().numpy(), self.spline_postacts[l][:, j, i][rank].cpu().detach().numpy(), color="blue", s= 2)
                    plt.gca().spines[:].set_color(color)

                    plt.savefig(f'{folder}/sp_{l}_{i}_{j}.png', bbox_inches="tight", dpi=600)
                    plt.close()
